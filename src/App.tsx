...
import { name as LOCALE_DE } from "dayjs/locale/de";
...
function Date(): ReactElement {
  ...
  return (
    <>
      <h1 className={"weekday"}>{date.locale(LOCALE_DE).format("dddd")}</h1>
      <div className={"date"}>
        {date.locale(LOCALE_DE).format("D. MMM YYYY")}
      </div>
    </>
  );
}
...
async function getMessages(): Promise<Message[]> {
  ...
  if (process.env.NODE_ENV === "production") {
    encrypted = await axios.get("%PUBLIC_URL%/messages-encrypted.txt");
  } else {
    encrypted = await axios.get("/messages-encrypted.txt");
  }
